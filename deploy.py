...
try:
    ...
    nuget_pack = f"nuget pack src/Honeybee.UI/Honeybee.UI.csproj -Properties Configuration=Release_Rhino -Properties id=Honeybee.UI.Rhino -Version {BUILD_VERSION} -Verbosity detailed"
    subprocess.check_output(nuget_pack)
    nuget_push = f"nuget push Honeybee.UI.Rhino.{BUILD_VERSION}.nupkg -ApiKey {NUGET_API_KEY} -Source https://api.nuget.org/v3/index.json"
    subprocess.check_output(nuget_push)
